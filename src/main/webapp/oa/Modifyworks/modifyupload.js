...
(function () {
    ...
    var filecount = Number($("#Allcount").val())+1;
    ...
    function ajaxFileUpload() {
        ...
        var file = thisId;
        var filePath;
        (file == "a01") ? filePath = "/image" : filePath = "/file";
...
//文件删除
    ...
    function uploadResult(dataRes) {
        ...
        if (success == "true") {
            ...
            if (fileloc == "/image") {
                ...
                datapath = Loadimg(fileloc, fileName, filecount, myData);
            } else {

                datapath = Loadfile(fileloc, fileName, filecount, myData,size);
            }

            var jsondata = {"filename":fileName,"filepath":fileloc};
            var json_str = JSON.stringify(jsondata);
            filedataArray.push(json_str);
            $("#addfiledata").val(filedataArray);

            var _counts = filecount
            $("#" + datapath).click(function () {

                F_Filedelete(fileName, fileloc, _counts);
            });
    ...
    function Loadimg(path, name, _id, data) {
        ...
        var appendstr = '<div id= "' + _id + '" class="image_div" style=" position:relative;width: 130px;height: 130px;margin-left:20px;">\n' +
            '                        <div class="imageid" style="position:absolute;width:100px;height:100px;left: 0px;top:0px;margin: 0 0;border: 1px solid #666;">\n' +
            ...
            '                            <input style="display:none" id=loc' +filecount + ' name=loc' +filecount + ' value=' + data + '>\n' +
            '\n' +
            '                        </div>\n' +
    ...
    function Loadfile(path, name, _id, data,size) {
        ...
        var appendfile = '<div id="' + _id + '" class="" style=" position:relative;width: 300px;height: 130px;margin-left:20px;">\n' +
            ...
            '                            </div>\n' +
            '                            <input style="display:none" id=loc' + filecount + ' name=loc' + filecount + ' value=' + data + '>\n' +
            '                            <input style="display:none" id=fileSize' + filecount + ' name=fileSize' + filecount + ' value=' + filesize + '>\n' +
            '                        </div>\n' +
            '                    </div>';
