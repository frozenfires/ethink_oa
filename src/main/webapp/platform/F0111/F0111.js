...
function beforeClick( treeId, treeNode, clickFlag) {
 ...
 function show_ORGTree( treeId, treeNode, clickFlag){
	...
	function onBodyDown(event) {
		...
		if (!(event.target.id == "showORGTree" || event.target.id == "menuContent" || $(event.target).parents("#menuContent").length>0)) {
			hideMenu();
		}
	}
})();
